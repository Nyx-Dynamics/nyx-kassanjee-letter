...
def plot_sensitivity(scenarios):
    ...
    atten_max = [s[1]['IRR_atten_pct'].max() for s in scenarios]
    
    ax.bar(names, atten_max, color='lightgreen', edgecolor='black', alpha=0.8)
    ax.set_ylabel('Max IRR Attenuation (%)')
    ax.set_title('Sensitivity of IRR Attenuation to Model Parameters')
    ax.set_xticklabels(names, rotation=45, ha='right')
    
    plt.tight_layout()
    fig_path = os.path.join(FIG_DIR, 'sensitivity_attenuation.png')
    plt.savefig(fig_path, dpi=300)
    print(f"Saved sensitivity figure to: {fig_path}")
