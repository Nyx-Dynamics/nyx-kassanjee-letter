...
FRAMEWORK (Demidont A.C., extending Kassanjee 2012 / Gao 2021):
  ...
  Effective MDRI under structural censoring:
    Ω*(γ) = ∫₀^T P_R(t) · S_c(t) dt
          ≈ Ω · (1 − γτ/2)   for small γτ, approx exponential P_R

  Screening cohort observation probability (given recent infection):
    ρ_screen(γ)  ≈ exp(−γ · τ/2),   τ ≈ 173 d (LAg-EIA effective window midpoint)

  LEN-arm observation probability (given true infection during follow-up):
    ρ_LEN(γ, ret) ≈ ρ_retention · exp(−γ · d_visit/2),   d_visit/2 ≈ 22.5 d (13-wk visits)
    where ρ_retention is retention-weighted person-time fraction

  Joint IRR bias factor:
    B_IRR(γ, ret) = ρ_LEN / ρ_screen
...
df.to_csv("/home/claude/kassanjee_bias/kassanjee_bias_by_pop.csv", index=False)
print(f"\nSaved: /home/claude/kassanjee_bias/kassanjee_bias_by_pop.csv")
