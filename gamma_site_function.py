...
where severity_substrate is the Meyer/Kamitani parameterization from
city_outbreak_profile.py (city_to_model_params function), and M is a
...
def severity_multiplier(vs_pct, late_dx_pct, linkage_pct, idu_pct,
                         gini, poverty_pct):
    """Meyer/Kamitani-anchored severity multiplier M(X)."""
...
for jf, trial_label in [
    ...
    with open(f'/mnt/project/{jf}') as f:
        d = json.load(f)
...
# Save
gdf_sorted.to_csv('/home/claude/phase1c_v2/city_gamma_table.csv', index=False)
print(f"\nSaved: city_gamma_table.csv")
