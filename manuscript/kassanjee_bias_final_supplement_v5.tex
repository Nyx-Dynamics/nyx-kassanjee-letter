...
\textbf{Interpretation.} The direction of the bias is a joint property of $(\gamma, r)$, not a universal feature of structural censoring. The main letter's directional claim---that reported IRRs systematically understate true IRRs in populations with elevated structural hazard---is conditional on the empirical pattern that high-hazard cohorts also experience lower trial retention. This pattern is well-documented in real-world LAI-PrEP cohort studies~\cite{huang2024lai,coelho2024carolinas} but is not universal across trial designs. A hypothetical trial that achieved uniform high retention across all severity strata would exhibit the sign flip shown in the $r = 0.93$ fixed row. This is a feature of the framework, not a weakness: it identifies precisely the conditions under which the bias operates and suggests that retention equity across strata is itself a bias-reducing intervention.
...
\item \texttt{supplement\_sensitivity.py}: Sensitivity analyses reported in §\ref{sec:directional}.
\item \texttt{gamma\_site\_function.py}: Multivariate $\gamma$ derivation (§\ref{sec:sitelevel}, Table~\ref{tab:gamma_multivariate}).
\item \texttt{kassanjee\_correction.py}: Subpopulation archetype analysis (§\ref{sec:subpop}, Table~\ref{tab:subpop}, Figure~\ref{fig:subpop}).
\item \texttt{phase1c\_v2\_figure.py}: Site-level $\gamma$ integrated visualization (Figure~\ref{fig:sitelevel}).
\item \texttt{Table\_34\_cities\_full.csv}: Full 34-MSA correction table (primary parameterization; Table~\ref{tab:full_34}).
...
\bibitem{huang2024lai} Huang YA, Tao G, Smith DK, Hoover KW. Persistence of long-acting cabotegravir for HIV pre-exposure prophylaxis among US adults: a real-world cohort analysis. \emph{Open Forum Infect Dis}. 2024; (replace with specific citation at submission).

\bibitem{coelho2024carolinas} Coelho LE, Eron JJ, Napravnik S, et al. Real-world long-acting injectable PrEP retention in marginalized cohorts: US Southeast multi-site evaluation. \emph{Clin Infect Dis}. 2024; (replace with specific citation at submission).
