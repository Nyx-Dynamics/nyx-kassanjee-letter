...
% Silence harmless LaTeX check warning
\usepackage{silence}
...
For each MSA, ordinary least squares regression of annual diagnosis counts on calendar year tested for monotonic linear trends. Mann--Kendall non-parametric trend tests were additionally applied to aggregate stratum time series. Pre-EHE (2014--2018) and post-EHE (2019--2023) means within each MSA were compared by Wilcoxon signed-rank test on within-MSA mean differences, yielding the median $\Delta = -14.8$ cases/year ($p < 0.0001$, $n = 35$ MSAs) reported in main-letter §4.6. Test-retest reliability was assessed by Pearson correlation between odd-year and even-year aggregates, yielding $r = 0.999$ for total diagnoses and $r = 0.89$ for IDU share. The MSA-level AR(1) autocorrelation coefficient $\phi = +0.04$ was estimated using the lag-1 autocorrelation of linearly detrended annual total diagnoses, averaged across the 35 MSAs; year-to-year residuals are essentially uncorrelated after removing the secular trend, consistent with measurement-noise rather than structural persistence.

Aggregate state-level and MSA-level post-EHE IDU-share slopes were computed as within-unit pre-EHE vs post-EHE differences in IDU-attributed diagnosis share. Aggregate state-level mean change was $+0.12$ percentage points per year ($p = 0.45$); aggregate MSA-level mean change was $-0.05$ percentage points per year ($p = 0.47$). Neither slope is statistically distinguishable from zero. Geographic redistribution of PWID HIV burden across 2014--2023 is detectable instead at the stratum level (§S8.3), as absolute diagnosis-count change within stratum membership rather than as a shift in pooled IDU-share trends. This pattern---pronounced stratum-level redistribution against null aggregate IDU-share slopes---is the longitudinal-surveillance analog of the cross-sectional calibration-to-deployment mismatch derived in main-letter §4: aggregate metrics calibrated against pooled national trends systematically fail to detect the structural redistribution that population-stratified analysis reveals.

\subsection{Van Handel overlay construction}

The 220 vulnerable-county FIPS codes were parsed from the CDC supplementary material accompanying Van Handel et al.\ (\emph{MMWR} 2016)~\cite{vanhandel2016} and cross-referenced against the AIDSVu County NewDX 2014--2023 panel using a county-to-MSA crosswalk. Three strata were defined: Stratum~A (counties inside the 35 EHE-priority MSAs, $n = 59$), Stratum~B (CDC-220 vulnerable counties outside the 35 EHE-priority MSAs; year-to-year reporting set varies 114--130 counties, median 125, reflecting AIDSVu cell-size suppression below 5 cases at the county-year level), and Stratum~C (all other US counties; year-to-year reporting set varies 1{,}868--1{,}964, median 1{,}897, reflecting the same cell-size suppression). The CDC-220 list shows near-clean partition against the 35 EHE-priority MSAs: 219 of 220 lie outside the MSAs, with the single exception of Wake County, NC (Raleigh~MSA, FIPS 37183, Van Handel rank 110), which is already a member of Stratum~A. Under the MSA-takes-precedence rule used throughout, Stratum~B therefore draws from a maximum of 219 candidate counties after subtraction of Wake County, with the year-by-year reporting set further constrained by cell-size suppression. Stratum-aggregate annual total dx, IDU dx, and IDU share were computed and tested for monotonic trend by Mann--Kendall. The canonical stratum-level absolute IDU-diagnosis trajectories 2014~$\to$~2023 are: Stratum~A 656~$\to$~636 ($-3.0\%$), Stratum~B 46~$\to$~46 (flat across endpoints, with intermediate peak 99 in 2019 and trough 43 in 2020), Stratum~C 541~$\to$~641 ($+18.5\%$). Stratum~C carries the absolute redistribution; Stratum~A's modest decline is the EHE-period attribution.

Note that Stratum~B's small absolute IDU-dx aggregate (46 cases across the median 125 reporting counties at both 2014 and 2023 endpoints, with intermediate peak 99 in 2019) reflects per-county AIDSVu cell-size suppression below 5 cases, which dominates the year-to-year share volatility for that stratum. This is itself diagnostic: the 2012--2013 Van Handel vulnerability geography produces a candidate 219-county set (220 minus Wake County NC) whose actual year-to-year reporting envelope of 114--130 counties carries insufficient absolute incidence in 2014--2023 to anchor stable trend inference, which supports rather than undermines the framing that point-in-time vulnerability indices are unstable surface proxies for the underlying barrier substrate $U$. The 2019 peak followed by 2020 collapse and 2022 partial rebound is the cleanest stratum-level signature of structural disruption in the panel.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.95\textwidth]{Fig_S3_kassanjee_invariance.png}
...
\label{fig:invariance_S3}
\end{figure}


\subsection{Comparison with prior national-scale modeling of post-COVID HIV outcomes}

The COVID-19 disruption window described in §4.6 motivates comparison with prior national-scale assessments of pandemic effects on U.S. HIV outcomes. Viguerie et al. \cite{viguerie2024} used the HIV Optimization and Prevention Economics (HOPE) compartmental model to project pandemic-related changes in HIV incidence through 2024, finding a transient incidence decline of 4.4\% during 2020–2021 driven by reductions in transmission behavior, followed by a partial rebound of 2.0\% during 2022–2024 (an average of approximately 713 additional incident infections per year above the no-pandemic counterfactual). Their analysis is the principal published national-scale assessment of pandemic effects on U.S. HIV incidence and serves as an important reference framework. Our stratum-disaggregated surveillance findings are broadly consistent with the 2020–2021 incidence pattern they report and provide a longitudinal complement to their 2022–2024 projection.
Several features of the surveillance data warrant consideration when interpreting the 2022–2024 projection. As described in Viguerie et al. (Table 1) \cite{viguerie2024}, the model's post-pandemic projection rests on the calibration choice that HIV testing rates, ART and PrEP initiation, and partner-count behavioral parameters return to 2019 baseline values in 2023. This is a reasonable simplifying assumption at the national-aggregate level, but our stratified longitudinal data suggest that the baseline-recovery assumption holds asymmetrically across the U.S. epidemic. In 2023, IDU-attributed diagnoses in CDC-220 vulnerable counties outside EHE-priority MSAs (Stratum B) reached only 30\% of the pre-COVID linear trajectory (46 observed vs. approximately 153 expected), and in other U.S. counties (Stratum C) reached 74\% (641 observed vs. approximately 866 expected). EHE-priority MSAs (Stratum A), by contrast, exceeded their pre-COVID trajectory in 2023 (636 vs. approximately 595 expected). This stratum-divergent recovery pattern is consistent with structural disruption that persisted beyond the acute pandemic window — including changes in Medicaid coverage beginning April 2023, harm-reduction service attrition in non-EHE jurisdictions, and continued public-health workforce capacity gaps — and that disproportionately affects populations outside EHE-priority MSAs.
Three additional considerations are relevant for interpreting the 2022–2024 incidence projection. First, HIV outbreaks among people who inject drugs were documented during and after the acute COVID-19 disruption period, including in northeastern Massachusetts \cite{alpren2020,cranston2019}, West Virginia \cite{hershow2022}, and other settings. Localized stochastic burst-incidence dynamics are difficult to represent in compartmental models that assume well-mixed populations within each stratification compartment. Second, methamphetamine use rose substantially during the pandemic, particularly among MSM \cite{carrico2020}, and methamphetamine-associated HIV transmission risk is well-documented \cite{cohen2022}. Third, geospatial concentration of vulnerable populations — into homeless encampments, single-room-occupancy housing, and consolidated harm-reduction service catchments — increased effective network density in PWID communities during 2020–2023 \cite{hassan2022}. The behavioral and structural shifts captured in these observations are not directly represented in the 2020-calibrated input parameters propagated through compartmental dynamics to 2023–2024.
These observations are complementary rather than contradictory to Viguerie et al.'s analysis. National-scale compartmental projections and stratified longitudinal surveillance offer different windows onto the same disrupted epidemic, and read together they provide a more complete picture than either view alone. The methodological observation we draw from the comparison — that cross-sectional calibration inputs propagated forward through compartmental dynamics may incompletely capture stratum-specific structural disruption that continues past the calibration period — is consistent with the calibration-to-deployment framework that motivates the present manuscript and applies, in different mechanistic forms, to both biomarker-based and model-based incidence estimators.

\subsubsection{Counterfactual methodology and stratum-level deficit derivation}\label{sec:covid_methodology}

\paragraph{Pre-COVID baseline regression and counterfactual extrapolation.} For each stratum $j \in \{A, B, C\}$, the pre-COVID trajectory is modeled as an ordinary least squares regression of annual IDU-attributed HIV diagnosis count on calendar year, fit to the 2014--2019 observations only (6 data points per stratum):
\begin{equation}
\hat{y}_j^{\mathrm{pre}}(t) = \hat\alpha_j + \hat\beta_j t, \quad t \in \{2014, \ldots, 2019\}
\end{equation}
The estimated slope $\hat\beta_j$ and intercept $\hat\alpha_j$ characterize the pre-COVID trajectory; the linear-trend assumption is consistent with the EHE-launch timing (February 2019) and avoids contaminating the pre-COVID baseline with post-EHE policy effects beyond 2019. The fit is propagated linearly to 2020--2023 to produce the counterfactual:
\begin{equation}
\hat{y}_j^{\mathrm{cf}}(t) = \hat\alpha_j + \hat\beta_j t, \quad t \in \{2020, \ldots, 2023\}
\end{equation}

\paragraph{Cumulative caveat-window deficit.} The COVID caveat window is defined as 2020--2022. This window encompasses the acute pandemic-era surveillance disruption (March 2020 onward), the year of partial healthcare recovery (2021), and the year of Medicaid unwinding initiation and ongoing operational disruption (2022). The cumulative caveat-window deficit for stratum $j$ is
\begin{equation}
D_j^{\mathrm{cw}} = \sum_{t=2020}^{2022} \left[\hat{y}_j^{\mathrm{cf}}(t) - y_j^{\mathrm{obs}}(t)\right]
\end{equation}
A positive $D_j^{\mathrm{cw}}$ indicates that observed diagnoses fell below the pre-COVID trajectory; a negative value indicates overshoot. Ninety-five percent confidence intervals on $D_j^{\mathrm{cw}}$ are derived from the OLS residual variance of the pre-COVID fit propagated to the 3-year extrapolation horizon under standard parametric bootstrap.

\paragraph{Post-caveat single-year deficit.} The 2023 single-year deficit is
\begin{equation}
D_j^{2023} = \hat{y}_j^{\mathrm{cf}}(2023) - y_j^{\mathrm{obs}}(2023)
\end{equation}
quantifying whether the disruption signal has resolved by 2023 (consistent with the pre-COVID trajectory) or persisted beyond the active surveillance-disruption window (consistent with continuing structural disruption).

\paragraph{Stratum-A trajectory and counterfactual.} The pre-COVID 2014--2019 trajectory in Stratum~A is monotonically declining: $\hat\beta_A = -5.89$ cases/year, $r^2 = 0.40$, $p = 0.18$. The counterfactual at 2023 is $\hat{y}_A^{\mathrm{cf}}(2023) = 595$ cases against an observed 636. The cumulative caveat-window deficit is $D_A^{\mathrm{cw}} = +89$ cases (95\% CI: $-101$, $+197$), which is not statistically distinguishable from zero. The 2023 single-year deficit is $D_A^{2023} = -39$ (i.e., observed exceeds counterfactual; overshoot of the declining pre-COVID expectation). This pattern is consistent with deferred-diagnosis catch-up rather than continuing structural disruption: the population absorbed the 2020 reduction in screening capacity, then over-recovered to the pre-COVID expectation. The Stratum~A reading is also a sanity check on the counterfactual methodology, because the resourced EHE-priority MSAs are the stratum whose pre-COVID trajectory was already declining (consistent with the EHE outcomes literature) and the absence of statistical disruption is the expected null-hypothesis result.

\paragraph{Stratum-B trajectory and counterfactual.} The pre-COVID trajectory in Stratum~B is sharply ascending: $\hat\beta_B = +13.60$ cases/year, $r^2 = 0.76$, $p = 0.023$. The counterfactual at 2023 is $\hat{y}_B^{\mathrm{cf}}(2023) = 153$ cases against an observed 46. The cumulative caveat-window deficit is $D_B^{\mathrm{cw}} = +185$ cases (95\% CI: $+74$, $+294$), corresponding to a 186.5\% sustained shortfall against the pre-COVID trajectory's 2019 baseline of 99. The 2023 single-year deficit is $D_B^{2023} = +107$ cases. The persistence of a single-year deficit nearly as large as the entire 2020--2022 cumulative window indicates that the disruption signal in Stratum~B has not resolved; the population's pre-COVID escalation has been replaced by a flat-low regime that the linear counterfactual cannot describe. Interpretively, the 2019 peak of 99 cases followed by collapse to 43 in 2020 and 46 in 2023---across the 114--130 reporting counties of Stratum~B---is the cleanest stratum-level signature of structural disruption in the panel, consistent with documented surveillance-engagement loss in CDC-220 vulnerable counties through the fentanyl-era opioid landscape.

\paragraph{Stratum-C trajectory and counterfactual.} The pre-COVID trajectory in Stratum~C is ascending: $\hat\beta_C = +33.37$ cases/year, $r^2 = 0.71$, $p = 0.036$. The counterfactual at 2023 is $\hat{y}_C^{\mathrm{cf}}(2023) = 866$ cases against an observed 641. The cumulative caveat-window deficit is $D_C^{\mathrm{cw}} = +361$ cases (95\% CI: $+18$, $+528$), corresponding to a 48.4\% shortfall against the 2019 baseline of 745. The 2023 single-year deficit is $D_C^{2023} = +225$ cases, approximately 74\% of the counterfactual trajectory's 2023 projection: the observed value is 26\% short of where the pre-COVID trend would have placed it. Like Stratum~B, the 2023 single-year deficit is a substantial fraction of the cumulative caveat-window deficit, indicating that the disruption has not resolved.

\paragraph{2020 stratum-decline gradient.} An independent diagnostic of the disruption gradient is the magnitude of the acute 2020 decline in each stratum, compared to the national aggregate. The national decline in newly diagnosed HIV cases from 2019 to 2020 was approximately $-17\%$~\cite{dinenno2022}. The stratum-specific 2020 declines are: Stratum~A $-13\%$ (factor 0.76 of national), Stratum~C $-33\%$ (factor 1.96 of national), Stratum~B $-57\%$ (factor 3.33 of national). The gradient is monotone in structural vulnerability: more structurally vulnerable strata sustained proportionally larger 2020 declines, with Stratum~B (the CDC-220 vulnerable, non-MSA counties) experiencing more than three times the national-aggregate decline. This gradient is a separable test of the structural-functions framework: the framework predicts that surveillance-disruption shocks should propagate differentially through the strata as a function of the latent barrier substrate $U$, and the observed gradient is consistent with that prediction without being calibrated against it.

\paragraph{Caveat window conservatism and the post-caveat 2023 reading.} The 2020--2022 caveat window over which causal interpretation is constrained is a deliberately conservative choice; many surveillance disruptions associated with COVID-19 had largely resolved by 2022, and the 2023 data are subject only to the residual operational disruptions persisting into the post-acute-COVID period. The fact that 2023 single-year deficits in Strata~B and C remain large fractions of their counterfactual projections (30\% and 74\% of trajectory, respectively) is therefore not attributable to active surveillance disruption; it is empirical evidence that the structural shift initiated during the COVID-era period has not reverted. The contrast with Stratum~A---whose 2023 observation overshoots its declining pre-COVID counterfactual---further sharpens this reading: in the stratum where structural capacity remained intact, the disruption was absorbed and reversed; in the strata where structural capacity was already marginal, the disruption initiated a regime change that the linear pre-COVID model cannot describe. The interpretive contrast with the Viguerie et al.\ 2024 HOPE counterfactual model, which implicitly assumed reversion to 2019 baseline as the post-caveat reading, is developed in §\ref{sec:viguerie}.

\subsubsection{Comparison with Viguerie et al.\ (2024) HOPE counterfactual model}\label{sec:viguerie}

Viguerie and colleagues~\cite{viguerie2024} reported a counterfactual model (the HIV Optimization and Prevention Economics, or HOPE, model) projecting the impact of COVID-19 on US HIV diagnoses and incidence over 2020--2024, with the modeling assumption that downstream input parameters return to 2019 baseline values by 2023--2024 as pandemic-era disruptions resolved. The model produced national-aggregate trajectory projections consistent with surveillance reports through 2022 and forecast a near-complete return to pre-pandemic incidence by 2024.

The stratified-longitudinal evidence developed in §\ref{sec:covid_methodology} departs from the HOPE projection at the post-caveat reading. Where HOPE projected reversion to 2019 baseline across the national aggregate, the stratum-stratified panel shows reversion in Stratum~A (overshoot of the declining counterfactual) but non-reversion in Strata~B and C (2023 single-year deficits remain $30\%$ and $74\%$ of their respective counterfactual trajectories). At the national-aggregate level, the Stratum~A overshoot partially offsets the Strata~B+C shortfall, producing an aggregate trajectory that is reasonably consistent with the HOPE projection while obscuring the stratum-divergent regime change. The HOPE model is consequently calibrated against a partition of the structural-vulnerability landscape that does not appear in the national-aggregate output to which its inputs are fit.

This departure is not a critique of the HOPE model on its own terms; the model was constructed to project national-aggregate trajectories and accomplished that task within the surveillance horizon then available. It is, rather, an instance of the broader calibration-to-deployment mismatch developed throughout the present letter: cross-sectional or pooled longitudinal calibration inputs derived from national-aggregate observation are systematically uninformative about the stratum-level regime changes that drive the operational landscape for HIV prevention in the most structurally vulnerable populations. The HOPE model's input parameters were calibrated against pooled national trends; the model's outputs faithfully reproduce those pooled trends; the populations the present analysis identifies as bearing the convergence-product risk (§5.1) are the populations whose trajectories diverge from the pooled national reading in the stratum-stratified panel. Both findings are simultaneously true. The stratified-longitudinal analytic posture developed here is the complementary frame that surfaces the stratum-divergent regime change the aggregate models cannot, in principle, detect.

The pattern of methodological complementarity is itself the generalization the present letter develops. Cross-sectional Kassanjee-style recency calibration and pooled-longitudinal HOPE-style counterfactual projection are both well-formed analyses against the calibration inputs they were designed for. Both fail in the same systematic direction when applied to populations whose structural substrate $U$ has shifted between calibration and deployment, and both fail at the same epistemic location: aggregating across heterogeneous structural strata produces summary statistics whose information content collapses precisely at the geographic and population margins where deployment decisions have the largest operational consequences. The stratified-longitudinal posture is a corrigible alternative to both.
\subsection{Data and code availability}\label{sec:S8_data_avail}\label{sec:code_data}

All data-processing and analysis code is available at \url{https://github.com/Nyx-Dynamics/nyx-kassanjee-letter} (Zenodo DOI: \href{https://doi.org/10.5281/zenodo.20344293}{10.5281/zenodo.20344293}). The repository includes:
\begin{itemize}
...
\item \texttt{rebuild\_figures.py}: Main-letter Figure~2 (stratum trajectories) and Figure~3 (COVID counterfactual) generation; Supplement Figure~S3 (Kassanjee invariance scatter) reuses the v8 main-letter Figure~3 image source.
\item \texttt{compute\_covid\_deficit.py}: OLS pre-COVID counterfactual fits and cumulative-deficit derivations per stratum (§\ref{sec:covid_methodology}).
...
\item \texttt{longitudinal addition data}: The panel CSVs, Van Handel parser output, stratum aggregates, and invariance-test outputs are deposited alongside the main analysis at \url{https://github.com/Nyx-Dynamics/nyx-kassanjee-letter}. The relevant files are listed in §\ref{sec:code_data}. 
\end{itemize}
...
\bibitem{alpren2020} Alpren C, Dawson EL, John B, et al. Opioid use fueling HIV transmission in an urban setting: an outbreak of HIV infection among people who inject drugs---Massachusetts, 2015--2018. \emph{Am J Public Health}. 2020;110(1):37--44.

\bibitem{cranston2019} Cranston K, Alpren C, John B, et al. Notes from the field: HIV diagnoses among persons who inject drugs---northeastern Massachusetts, 2015--2018. \emph{MMWR Morb Mortal Wkly Rep}. 2019;68(10):253--254.

\bibitem{hershow2022} Hershow RB, Wilson S, Bonacci RA, et al. Notes from the field: HIV outbreak during the COVID-19 pandemic among persons who inject drugs---Kanawha County, West Virginia, 2019--2021. \emph{MMWR Morb Mortal Wkly Rep}. 2022;71(2):66--68.

\bibitem{carrico2020} Carrico AW, Horvath KJ, Grov C, et al. Double jeopardy: methamphetamine use and HIV as risk factors for COVID-19. \emph{AIDS Behav}. 2020;24(11):3020--3023.

\bibitem{cohen2022} Cohen SE, Hassan A, Berry M, et al. Methamphetamine-associated HIV transmission among men who have sex with men: surveillance and prevention implications. \emph{Drug Alcohol Depend Rep}. 2022;5:100107.

\bibitem{hassan2022} Hassan A, Blumenthal JS, Dube MP, et al. Effect of COVID-19 on PrEP persistence and HIV testing among PrEP users. \emph{J Subst Abuse Treat}. 2022;134:108593.
...
\bibitem{demidont2026fw} Demidont AC. Finite Prevention Windows for HIV Post-Exposure Prophylaxis: Irreversible Proviral Integration Defines Route-Specific and Population-Level Intervention Limits. \emph{Preprints}. April 2026. doi: 10.20944/preprints202601.1090.v2. Under peer review at \emph{Science Advances}.

\bibitem{demidont_barriers_synergy} Demidont AC. Structural Barriers, Stochastic Avoidance, and Outbreak Risk in HIV Prevention for People Who Inject Drugs. \emph{Preprints}. January 2026. doi:10.20944/preprints202601.0948.v1. Under peer review at \emph{BMC Public Health}.
