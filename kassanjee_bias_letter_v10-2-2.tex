...
\usepackage{url}
% Cross-document refs to the supplement (requires kassanjee_bias_supplement_v8.aux to exist)
\usepackage{xr-hyper}
\externaldocument{kassanjee_bias_supplement_v10}

% --- Line-breaking tolerance (prevents minor over/underfull boxes without changing text) ---
...
\noindent\textbf{Background.} Cross-sectional HIV incidence estimation via recent-infection testing algorithms (RITAs) anchors counterfactual-controlled pre-exposure prophylaxis (PrEP) efficacy trials. The Kassanjee estimator assumes that every recently-infected individual within the recency window is equally present at screening. Populations experiencing structural censoring---competing-risk hazards from overdose, incarceration, and displacement---violate this assumption systematically and directionally.
\medskip

\noindent\textbf{Methods.} We derive the effective mean duration of recent infection under structural censoring and the joint bias factor on the reported incidence rate ratio. The standard 90-day no-prior-testing eligibility criterion, common to Phase 3 PrEP trials, operates on the same axis that drives the competing-risk hazard. Applied to 34 high-burden US MSAs using AIDSVu 2023 data and anchored against the 2014--2023 county-level panel.
\medskip

\noindent\textbf{Results.} Four independent longitudinal analyses ($N = 350$ MSA-years) converge on the reframing: temporal stability (AR(1) $\phi = +0.04$; test-retest $r = 0.999$ for total diagnoses, $r = 0.89$ for IDU share); a pre-EHE/post-EHE within-MSA Wilcoxon break-point (median $\Delta = -14.8$ cases/year, $p < 0.0001$); stratum-level geographic redistribution of IDU diagnoses against null aggregate slopes (Stratum~A EHE-priority MSAs $-3.0\%$, Stratum~C all other counties $+18.5\%$); and a Kassanjee-correction invariance test across 34 MSAs (Spearman $\rho = 0.9979$, identical optimal cascade policies in 34/34 cities).
\medskip

\noindent\textbf{Conclusions.} The Kassanjee/Gao estimator produces systematically biased point estimates in populations with elevated structural censoring, and the bias is structurally guaranteed when eligibility criteria select on the testing-engagement axis. Correction requires explicit hazard modeling from surveillance data; the framework is reproducible from public data and generalizes to any RITA-based trial with analogous eligibility structure.
\medskip

\noindent\textbf{Keywords:} HIV pre-exposure prophylaxis (PrEP); cross-sectional HIV incidence; recent infection testing algorithm (RITA); Kassanjee estimator; people who inject drugs (PWID); counterfactual clinical trial design
\newpage
...
We derive the effective MDRI under structural censoring, $\Omega^*(\gamma)$; derive the joint bias factor on the reported incidence rate ratio incorporating both screening-cohort and intervention-arm observation probabilities; quantify the amplification introduced by the testing-exclusion selection; apply the framework to 34 high-burden US metropolitan areas using publicly available AIDSVu 2023 surveillance data \cite{aidsvu2023}, with late-diagnosis percentage as the empirical proxy for testing-avoidance hazard; and empirically anchor the structural-functions reframing using the 2014--2023 AIDSVu county-level panel. The correction can be implemented prospectively in any RITA-based trial SAP without access to proprietary data; the framework applies to any cross-sectional incidence estimator with analogous eligibility structure.
...
Under a piecewise-constant hazard approximation with $\gamma$ constant over the recency window and an exponential approximation $P_R(t) \approx P_0 \exp(-t/\tau)$ with $\tau \approx \Omega$ for the Sedia LAg-EIA calibration \cite{sedia} (we treat $\gamma$ as independent of age-of-infection within the recency window and assume no differential hazard between recently-infected and uninfected screening-cohort members; consequences of relaxing both assumptions are explored in Supplement \S S1.2), a closed-form correction obtains:
\begin{equation}
...
Late-diagnosis percentages, viral suppression rates, linkage-to-care rates, IDU prevalence, Gini coefficients, and poverty rates for 34 high-burden US metropolitan areas were obtained from the AIDSVu 2023 downloadable dataset \cite{aidsvu2023}. The national late-diagnosis reference was taken as 20\%. Per-city competing-risk hazard was parameterized as in Equation 10, with $\gamma_{\mathrm{base}} = 5 \times 10^{-4}$ per day \cite{cdc_hiv2023,cdc_nhbs2023,bjs2023} and $\alpha = 1.2$. The 90-day selection amplification factor was set at 1.5. Intervention-arm retention was modeled as a decreasing function of late-diagnosis severity:
\begin{equation}
...
A trial reporting IRR = 0.04 (96\% efficacy) in an aggregate analysis across cohorts with $\gamma_{\mathrm{enrolled}}$ concentrated at the low-severity end of the distribution cannot have that IRR applied without correction to deployment populations at the high-severity end. The Kassanjee bias factor differs by approximately 3 percentage points across the full AIDSVu range---small relative to the primary biological constraints on transportability \cite{demidont_finite} but, as developed below and in \S 5, non-trivial for populations at the margin of clinical and regulatory decisions.
...
All code and data-processing scripts, including the longitudinal additions (panel construction, EHE break-point, state-vs-MSA divergence, Van Handel overlay, and Kassanjee correction invariance test), are available at \url{https://github.com/Nyx-Dynamics/nyx-kassanjee-letter} (Zenodo DOI: 10.5281/zenodo.20344293). The AIDSVu 2023 dataset and 2014--2023 county-level panel files are publicly available at \url{https://aidsvu.org}. No individual-level data were used.
